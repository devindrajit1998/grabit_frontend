...
import { closeModal } from "../redux/slice/CommonSlice";
const baseUrl = import.meta.env.VITE_BASE_URL;

export default function ProductModal() {
  const modalOpen = useSelector((state) => state.commonSlice.data.isOpen);
  const modalData = useSelector(
    (state) => state.productSlice.product?.attributes
  );
  const modalState = useSelector((state) => state.productSlice.status);
  ...
  const mainPrice = modalData?.price;
  const discount = modalData?.offer / 100;
  const discountedPrice = mainPrice * discount;
  const finalPrice = (mainPrice - discountedPrice).toFixed(0);
  // console.log("finalPrice", finalPrice);
  return (
    <>
      {modalState === "successful" && (
        <>
          {modalOpen && (
            <>
              <div
                className="gi-modal-overlay w-full h-screen fixed top-0 left-0 z-[30] bg-[#000000b3]"
                            ...
                            <div className="single-product-scroll h-full">
                              <div className="single-slide h-full flex items-center zoom-image-hover">
                                <img
                                  ...
                                  src={
                                    baseUrl +
                                    modalData?.thumbnail.data.attributes.url
                                  }
                                  alt=""
                                />
                              </div>
                          ...
                          <div className="quickview-pro-content">
                            <h5 className="gi-quick-title">
                              <a
                                ...
                                className="mb-[15px] block text-[#4b5966] text-[22px] leading-[1.5] font-medium max-[991px]:text-[20px] clamped_2"
                              >
                                {modalData?.title}
                              </a>
                            </h5>
                            <div className="gi-quickview-rating flex mb-[15px]">
                              <i className="gicon gi-star fill text-[14px] text-[#f27d0c] mr-[5px]" />
                              <i className="gicon gi-star fill text-[14px] text-[#f27d0c] mr-[5px]" />
                              <i className="gicon gi-star fill text-[14px] text-[#f27d0c] mr-[5px]" />
                              <i className="gicon gi-star fill text-[14px] text-[#f27d0c] mr-[5px]" />
                              <i className="gicon gi-star text-[14px] text-[#777] mr-[5px]" />
                            </div>
                            <div className="gi-quickview-desc mb-[10px] text-[15px] leading-[24px] text-[#777] font-light clamped_3">
                            ...
                            <div className="gi-quickview-qty mt-[15px] inline-flex">
                              ...
                              <div className="gi-quickview-cart">
                                ...
                                >
                                  <i className="fi-rr-shopping-basket text-[14px] leading-[0] mr-[5px]" />
                                  Add To Cart
                                </button>
                              </div>
                            </div>
